...
const GetCityDB = async (city, token) => {
  try {
    console.log(city);
...
const GetCity = async (city, token) => {
  try {
    console.log(city);
    const response = await fetch(`http://localhost:5127/SolarWatch/Getcurrent?name=${city}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${token}`,
...
const PostCity = async (city, token) => {
  try {
    ...
    GetCityDB(city, token)
      ...
      .catch((error) => {
        ...
        GetCity(city, token)
          ...
          .catch((error) => {
            setLoading(false);
            alert(`There is no City: ${city} found on WWW!`);
            console.error("Error occurred during login:", error);
          });
      });
  };
